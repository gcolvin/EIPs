...
In 2015 the lead author set out to write a simple transpiler from EVM code to native machine code.  The first step is to trace the control flow of the progam, and although the control flow may loop the trace must not.  But there was a problem -- as soon as a trace reaches a `JUMP` it can jump dynamically to any `JUMPDEST` in the progam.  Worst case contracts are half `JUMP`s and half `JUMPDEST`s, and the number of possible traces goes up quadratically with the contract size.  So `JUMP` and `JUMPI` are DoS surfaces for any static analysis of code on-chain, and a large obstacle for proofs of correctness off-chain.

Ever since then the present authors and others have sought to tame or remove `JUMP` and `JUMPDEST`.  [EIP-4200](./eip-4200.md) provides static `RJUMP` and `RUMPI` opcodes, and this proposal allows for the static use of `JUMP` and `JUMPI`.  The one dynamic use of `JUMP` which cannot be tamed is Turing's _subsidiary operations._  For that purpose we propose `RJUMPSUB` and `RETURNSUB` opcodes.
...
##### Contracts which begin with `0xEF` MUST be valid.
...
* The _data stack_ MUST be consistently aligned:
   ...
   * The _height_ MUST be the same for every backwards branch, that is,
     * a reachable path through a given `PC` where
       * `IP` is to `PUSH dst JUMP` or `PUSH dst JUMPI` and `dst - PC < 0` or where
       * `IP` is to `RJUMP rdst` or `RJUMPI rdst`  and `rdst < 0.` 
...
These changes do not affect the semantics of existing EVM code: with the caveat that bytes that would have been interpreted as valid jump destinations may now be interpreted as immediate data. Such code was always broken.  The semantics of existing or new opcodes is not affected by whether the contract begins with `0xEF` -- only their validity.
...
These changes introduce new flow control instructions.  They do not introduce any new security considerations. This EIP is intended to improve security by validating a higher level of safety for EVM code deployed on the blockchain.  The validation algorithm must be linear in time and space to not be a denial of service vulnerability.  The algorithm here makes one linear-time pass of the bytecode, and uses a stack of continuations that cannot exceed the number of `RJUMPI` instructions in the code.
