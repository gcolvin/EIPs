...
* `RJUMPSUB relative_offset` -- call to relative offset from `PC`.
...
3. Set `PC` to `PC + code_offset`.
   * The `code_offset` is relative to the beginning of the program.
   ...
   * Space after the code offset and its destination is reserved to the implementation.
      *  Implementation-reserved space is subject to use by future non-consensus standards. 
...
### **Contracts which begin with `0xEFFF` MUST be Valid:**
...
This is a purely semantic specification, placing no constraints on the syntax of code sections beyond being a sequence of opcodes and immediate data – a valid sequence is not a contiguous sequence of bytecode, it is a valid subgraph of the bytecode's full control-flow graph.  The EVM is a simple state machine.  We only promise that valid code will not, as it were, jam up the gears of the machine.
...
Clearly, the benefits of this efficiency are greater for programs that have been factored into smaller calls -- the overhead for calling and returning from a call with `RJUMPSUB` and `RETURNSUB` is only 8 gas, whereas the overhead for `JUMP` is 28 gas.
...
In this example the RJUMPSUB is on the last byte of code. When the call returns, it should hit the 'virtual stop' _after_ the bytecode, and not exit with error
