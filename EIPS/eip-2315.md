...
description: Two opcodes for efficient, safe, and static control flow.
...
In 2015 the lead author set out to write a simple compiler from EVM code to native machine code.  The first step was to trace the control flow of the progam, and although the control flow may loop the trace must not.  But there was a problem -- as soon as a trace reaches a `JUMP` it can jump dynamically to any `JUMPDEST` in the progam.  Worst case contracts are half `JUMP`s and half `JUMPDEST`s, and the number of possible traces goes up quadratically with the contract size.  So `JUMP` and `JUMPI` are DoS surfaces for any static analysis of code on-chain -- including optimizing, compiling, and validating EVM code -- and a large obstacle for proofs of correctness off-chain.  These are all of obvious value for efficient and correct EVM code, and they are all impeded by dynamic jumps.

Ever since 2015 the present authors and others have sought to tame or remove `JUMP` and `JUMPDEST`.  [EIP-4200](./eip-4200.md) provides static `RJUMP` and `RUMPI` opcodes, and this proposal allows only static use of `JUMP` and `JUMPI`.  The one dynamic use of `JUMP` which cannot be tamed is Turing's _subsidiary operations._  For that purpose we propose `RJUMPSUB` and `RETURNSUB` opcodes.
...
* The _data stack_ MUST be consistently aligned:
   ...
   * The _height_ MUST be the same for every backwards branch, that is,
     * a reachable path through a given `PC` where
       * `IP` points to `PUSH dst JUMP` or `PUSH dst JUMPI` and `dst - PC < 0` or where
       * `IP` points to `RJUMP rdst` or `RJUMPI rdst`  and `rdst < 0.` 
   * The _height_ MUST NOT exceed 1024.
