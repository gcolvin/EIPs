...
*Constraint 4, the data stack*, follows from Constraint 5.  Under path-independent offsets, an instruction's depth relative to its entry is a single number, so "finds at least as many items as it removes, on every path" reduces to the *demand* computation: the deepest an entry's own code reaches below its base, less the items each parent supplies at the point of entrance, propagated to the top level, which has nothing to supply.  This is a restatement, not an approximation; the limit only turns away demands that no stack could satisfy, which underflow on every path.

*Constraint 4, the return stack*, is where an earlier draft was sound but not complete.  It carried a *framed* flag along each path and rejected any instruction reached with the flag both set and clear.  That rejects a block shared between top-level code and a subroutine — a revert sequence, say — which contains no `RETURNSUB` and so satisfies the constraint on every path.  Framing as a property of *entries* is exact.  Call an entry *unframed* if it is the top level or is reached by a jump or fall-through from an unframed entry; a `CALLSUB` frames its callee, and a return point keeps its caller's framing.  The algorithm rejects exactly when an unframed entry contains a `RETURNSUB`.

* If it rejects, some unframed entry contains a `RETURNSUB`; unframed means a chain of jumps from the top level reaches that entry with no `CALLSUB` on it, so a path reaches that `RETURNSUB` on which every `CALLSUB` has returned: a violation.
* If some path reaches a `RETURNSUB` with no unreturned `CALLSUB`, delete its returned call–return pairs; what remains is a chain of jumps and fall-throughs from the start into the entry containing the `RETURNSUB`, through entries reached the same way, each unframed by definition.  So that entry is unframed and contains a `RETURNSUB`: the algorithm rejects.

The per-path rule proved a stronger property than the constraint states; the per-entry rule proves exactly it, at the cost of one fixpoint over the entries beside the demand propagation, and one field less in the per-instruction record.
